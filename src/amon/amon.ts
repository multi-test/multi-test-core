...
import {createBlankScales} from "./createBlankScales";
import {AmonAnswer as Answer, AmonScales as Scales} from "./types";

const validate = buildAnswersValidator<Answer>(220, ["+", "-"]);

type AmonScaleMeta = { scaleId: keyof Scales; indices: number[]; };

const meta: AmonScaleMeta[] = [
...
const scaleReducers: Array<IScaleReducer<Scales, Answer>> = meta.map((scaleMeta) => {
    let {indices, scaleId} = scaleMeta;
    let set = new Set<number>(indices);

    return function scaleReducer(scales: Scales, value: Answer, index: number): Scales {
        if (value === "+" && set.has(index)) {
            scales[scaleId] = Number(scales[scaleId]) + 1;
        }
...
const rootReducer: IScaleReducer<Scales, Answer> = combineReducers(scaleReducers);

function calculate(answers: Answer[]): Scales {
    return answers.reduce(rootReducer, createBlankScales(0));
}

export default function amon(answers: any[]): Scales {
