...
import {createBlankScales, IUSKScales} from "./scales";

const rootReducer = combineReducers<IUSKScales>([
    {
        scaleId: "Ио",
        sign: 1,
        indices: [2, 4, 11, 12, 13, 15, 16, 17, 19, 20, 22, 25, 27, 29, 31, 32, 34, 36, 37, 39, 42, 44],
    },
    {
        scaleId: "Ио",
        sign: -1,
        indices: [1, 3, 5, 6, 7, 8, 9, 10, 14, 18, 21, 23, 24, 26, 28, 30, 33, 35, 38, 40, 41, 43],
    },
...
const validate = buildAnswersValidator(44, [-3, -2, -1, 1, 2, 3]);

export default function usk(answers: any[]): IUSKScales {
    return validate(answers) ? normalize(calculate(answers)) : createBlankScales(NaN);
}