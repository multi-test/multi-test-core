...
import {createBlankScales, IMaddiAnswer, IMaddiScales} from "./scales";
...
const validate = buildAnswersValidator(45, [0, 1, 2, 3]);

export default function maddi(answers: IMaddiAnswer[]): IMaddiScales {
    return validate(answers) ? compute(calculate(answers)) : createBlankScales(NaN);
}
